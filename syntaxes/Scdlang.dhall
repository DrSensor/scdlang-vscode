...
let Include/from = λ(a: Type) -> λ(b: Map Text a) -> pattern.Include/from (keys a b)
let patterns = {
  transition = Include/from pattern.LineMatch (toMap Transition),
  comment = Include/from pattern.BlockMatch (toMap Comment)
}
...
in {
  ...
  patterns = patterns.comment # patterns.transition,
